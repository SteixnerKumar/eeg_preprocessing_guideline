...
function EEG = sk_eeg_interp(ORIEEG, bad_elec, method)
...
%         EEG       = pop_select(EEG, 'nochannel', badchans);
    ...
    %%% sk version
    tmpdata               = zeros(size(EEG.data,1), size(EEG.data,2));
    tmpdata(origoodchans, :) = EEG.data(origoodchans, :);
    %%%
...
%     tmpdata(origoodchans, :,:) = EEG.data;
    ...
    disp('Interpolating missing channels... Done.');
